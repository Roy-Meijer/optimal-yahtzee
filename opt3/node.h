...
public:
    ...
    Node(NodeType type = DICE_NODE, int scoreOnes = -1, int scoreTwos = -1, std::vector<Node*>* parent=nullptr, std::vector<Node*>* scoreOptions = nullptr, std::vector<Node*>* children = nullptr, bool childHandled = false, REROLL_TYPE reroll_decision = REROLL_NA, int nodeIndex = -1)
    : type(type), reroll_decision(reroll_decision), parent(parent), maxExpectedScore(-1.0), numChildren(0), children(children), scoreOptions(scoreOptions), childHandled(childHandled), nodeIndexDepth(nodeIndex), nodeIndexGlobal(nodeIndex), scoreOnes(scoreOnes), scoreTwos(scoreTwos) {
    }
    ...
    std::vector<Node*>* generateOutcomeNodes(Node* node)
    {
        ...
        if (node->scoreOptions == nullptr)
        {
            ...
            if (node->getType() == Node::NodeType::ROOT_NODE || node->getType() == Node::NodeType::SCORE_ROOT_NODE)
            {
                ...
                Node* score_11 = new Node(Node::OUTCOME_NODE, node->getScoreOnes(), node->getScoreTwos());
                Node* score_12 = new Node(Node::OUTCOME_NODE, node->getScoreOnes(), node->getScoreTwos());
                Node* score_22 = new Node(Node::OUTCOME_NODE, node->getScoreOnes(), node->getScoreTwos());
    ...
    void setNodeIndexDepth(int nid) {
        nodeIndexDepth = nid;
    }

    int getNodeIndexDepth() {
        return nodeIndexDepth;
    }

    void setNodeIndexGlobal(int ni) {
        nodeIndexGlobal = ni;
    }

    int getNodeIndexGlobal() {
        return nodeIndexGlobal;
    }
